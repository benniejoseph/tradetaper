...
import { MetaApiService, MT5AccountCredentials, HistoricalTradeFilter } from './metaapi.service';
import { CreateMT5AccountDto, CreateManualMT5AccountDto, UpdateMT5AccountDto, TradeHistoryUploadResponse } from './dto/mt5-account.dto';
...
import { AssetType, TradeDirection, TradeStatus } from '../trades/entities/trade.entity';
...
export class MT5AccountsController {
  ...
  @Post()
  async create(@Request() req, @Body() createMT5AccountDto: CreateMT5AccountDto) {
  ...
  @Post('manual')
  async createManual(@Request() req, @Body() createMT5AccountDto: CreateManualMT5AccountDto) {
  ...
  async connectAccount(@Request() req, @Param('id') id: string) {
    ...
    if (account.userId !== req.user.id) {
      throw new ForbiddenException('You do not have permission to connect this account');
    }
  ...
  async startStreaming(@Request() req, @Param('id') id: string) {
    ...
    if (account.userId !== req.user.id) {
      throw new ForbiddenException('You do not have permission to start streaming for this account');
    }
  ...
  async stopStreaming(@Request() req, @Param('id') id: string) {
    ...
    if (account.userId !== req.user.id) {
      throw new ForbiddenException('You do not have permission to stop streaming for this account');
    }
  ...
  async syncAccount(@Request() req, @Param('id') id: string) {
    ...
    if (account.userId !== req.user.id) {
      throw new ForbiddenException('You do not have permission to sync this account');
    }
  ...
  async importTrades(
    ...
    if (account.userId !== req.user.id) {
      throw new ForbiddenException('You do not have permission to import trades from this account');
    }
  ...
  async validateConnection(@Request() req, @Param('id') id: string) {
    ...
    if (account.userId !== req.user.id) {
      throw new ForbiddenException('You do not have permission to validate this account');
    }
    ...
    try {
      ...
      return { valid: false, message: `Connection validation failed: ${error.message}` };
    }
  }
  ...
  async uploadTradeHistory(
    ...
    if (account.userId !== req.user.id) {
      throw new ForbiddenException('You do not have permission to upload trade history for this account');
    }
    ...
    if (fileName.endsWith('.html') || fileName.endsWith('.htm')) {
      ...
      throw new BadRequestException('Unsupported file type. Please upload HTML or Excel (.xlsx) files only.');
    }

    try {
      // Parse the uploaded file
      const parseResult = await this.tradeHistoryParserService.parseTradeHistory(
        file.buffer,
        fileType,
        file.originalname
      );

      const { trades, accountBalance, accountCurrency, totalNetProfit, equity } = parseResult;
      ...
      const importResult = await this.tradesService.bulkImport(tradesDtos, req.user);
   ...
   */
  private convertMT5TradesToCreateTradeDto(parsedTrades: any[], accountId: string): CreateTradeDto[] {
    return parsedTrades.map(trade => {
      // Determine asset type based on symbol
      let assetType: AssetType = AssetType.FOREX; // Default for MT5
      if (trade.symbol.includes('USD') || trade.symbol.includes('EUR') || trade.symbol.includes('GBP') || trade.symbol.includes('JPY')) {
        assetType = AssetType.FOREX;
      }

      // Convert MT5 trade type to our trade direction
      const direction = trade.type === 'buy' ? TradeDirection.LONG : TradeDirection.SHORT;
      ...
      const dto: CreateTradeDto = {
        ...
        notes: trade.comment || `Imported from MT5 - Position ID: ${trade.positionId}`,
