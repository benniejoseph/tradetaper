...
export class TradeHistoryParserService {
   ...
   */
  async parseTradeHistory(buffer: Buffer, fileType: 'html' | 'xlsx', fileName: string): Promise<{ trades: ParsedTradeData[], accountBalance?: number, accountCurrency?: string, totalNetProfit?: number, equity?: number }> {
    ...
    try {
      if (fileType === 'html') {
        ...
        throw new BadRequestException('Unsupported file type. Only HTML and XLSX files are supported.');
      }
    } catch (error) {
      this.logger.error(`Failed to parse trade history file ${fileName}:`, error);
      throw new BadRequestException(`Failed to parse trade history: ${error.message}`);
    }
  }
   ...
   */
  private parseHTMLTradeHistory(buffer: Buffer): { trades: ParsedTradeData[], accountBalance?: number, accountCurrency?: string, totalNetProfit?: number, equity?: number } {
    ...
    while ((match = tableRowRegex.exec(html)) !== null) {
      ...
      if (!headerFound && rowText.includes('position') && rowText.includes('symbol') && rowText.includes('type')) {
        headerFound = true;
        ...
        cells.forEach((header, index) => {
          ...
          if (headerText.includes('price') && !headerText.includes('s / l') && !headerText.includes('t / p')) {
      ...
      if (headerFound && cells.length > 5) {
        ...
        if (positionId && symbol && (type === 'buy' || type === 'sell') && 
            !isNaN(openPrice) && !isNaN(closePrice) && 
            openTime && closeTime && !isNaN(profit)) {
   ...
   */
  private parseExcelTradeHistory(buffer: Buffer): { trades: ParsedTradeData[], accountBalance?: number, accountCurrency?: string, totalNetProfit?: number, equity?: number } {
    ...
    for (const sheetName of workbook.SheetNames) {
      ...
      for (let i = 0; i < Math.min(sheetData.length, 10); i++) {
        ...
        if (rowText.includes('balance') && !accountBalance) {
          ...
          for (let j = 0; j < row.length; j++) {
            ...
            if (cell && typeof cell === 'string' && cell.toLowerCase().includes('balance')) {
        ...
        if (rowText.includes('currency') && !accountCurrency) {
          for (let j = 0; j < row.length; j++) {
            const cell = (row[j] || '').toString();
            if (cell && typeof cell === 'string' && cell.toLowerCase().includes('currency')) {
      ...
      for (let i = 0; i < Math.min(sheetData.length, 10); i++) {
        ...
        const rowText = row.join(' ').toLowerCase();
        if (rowText.includes('position') && rowText.includes('symbol') && rowText.includes('type')) {
          headerRowIndex = i;
