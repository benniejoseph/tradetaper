...
export class AppController {
  ...
  async runMigration() {
    try {
      ...
      return {
        success: true,
        message:
          'Migration completed successfully - created all tables, sample trades, dropped and recreated usage_tracking table, added plan column',
        timestamp: new Date().toISOString(),
      };
    } catch (error) {
  ...
  async debugUsers() {
    try {
      ...
      console.log(
        '✅ Direct SQL user creation with hashed password:',
        testUserId,
      );
      ...
      try {
        ...
        if (testPriceId) {
          const session = await stripe.checkout.sessions.create({
            ...
            mode: 'subscription',
            success_url:
              'https://tradetaper-frontend-benniejosephs-projects.vercel.app/success',
            cancel_url:
              'https://tradetaper-frontend-benniejosephs-projects.vercel.app/cancel',
