...
export class AppController {
  ...
  @Post('debug-auth')
  async debugAuth(@Body() body: { email: string; password: string }) {
    try {
      console.log('🔍 Testing auth service step by step...');
      
      // Step 1: Check if auth service exists
      const AuthService = require('./auth/auth.service').AuthService;
      console.log('✅ AuthService class exists');
      
      // Step 2: Check if users service can find user by email
      const user = await this.subscriptionRepository.query(`
        SELECT id, email, password, "firstName", "lastName", "createdAt" FROM users WHERE email = $1;
      `, [body.email]);
      console.log('✅ User found:', !!user[0]);
      
      if (user[0]) {
        // Step 3: Test password validation manually
        const bcrypt = require('bcrypt');
  ...
  async simpleRegister(@Body() body: { email: string; password: string; firstName: string; lastName: string }) {
    try {
      console.log('🔍 Simple registration test...');
      
      // Check if user already exists
      const existingUser = await this.subscriptionRepository.query(`
      ...
      // Create user
      const newUser = await this.subscriptionRepository.query(`
        INSERT INTO users (email, password, "firstName", "lastName") 
        VALUES ($1, $2, $3, $4) 
