...
describe('NotesController', () => {
  ...
  const mockNote: NoteResponseDto = {
    id: 'uuid-note-1',
    title: 'Test Note',
    content: [{ id: 'block-1', type: 'text', content: { text: 'Some text' }, position: 0 }],
    tags: ['tag1', 'tag2'],
    createdAt: new Date(),
    updatedAt: new Date(),
    isPinned: false,
      ...
      providers: [
        {
          ...
          useValue: {
            ...
            findAll: jest.fn().mockResolvedValue({ notes: [mockNote], total: 1, limit: 20, offset: 0 }),
            findOne: jest.fn().mockResolvedValue(mockNote),
            update: jest.fn().mockResolvedValue(mockNote),
            remove: jest.fn().mockResolvedValue(undefined),
            togglePin: jest.fn().mockResolvedValue({ ...mockNote, isPinned: true }),
            getAllTags: jest.fn().mockResolvedValue(['tag1', 'tag2']),
            getCalendarNotes: jest.fn().mockResolvedValue([]),
            getStats: jest.fn().mockResolvedValue({ totalNotes: 1, totalWords: 10, totalReadingTime: 1, pinnedNotes: 0, notesWithMedia: 0, averageWordsPerNote: 10, mostUsedTags: [] }),
  ...
  describe('create', () => {
    it('should create a note', async () => {
      const createNoteDto: CreateNoteDto = {
        title: 'New Note',
        content: [{ id: 'block-2', type: 'text', content: { text: 'New content' }, position: 0 }],
      };
  ...
  describe('findAll', () => {
    it('should return an array of notes', async () => {
      const req = { user: mockUser } as any;
      expect(await controller.findAll({}, req)).toEqual({ notes: [mockNote], total: 1, limit: 20, offset: 0 });
  ...
  describe('update', () => {
    it('should update a note', async () => {
      ...
      const req = { user: mockUser } as any;
      expect(await controller.update(mockNote.id, updateNoteDto, req)).toEqual(mockNote);
      expect(service.update).toHaveBeenCalledWith(mockNote.id, updateNoteDto, mockUser.id);
    });
  });
  ...
  describe('togglePin', () => {
    it('should toggle the pin status of a note', async () => {
      const req = { user: mockUser } as any;
      expect(await controller.togglePin(mockNote.id, req)).toEqual({ ...mockNote, isPinned: true });
