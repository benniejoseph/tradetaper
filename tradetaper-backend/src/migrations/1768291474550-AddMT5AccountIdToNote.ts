...
export class AddMT5AccountIdToNote1768291474550 implements MigrationInterface {
  ...
  public async up(queryRunner: QueryRunner): Promise<void> {
    ...
    await queryRunner.query(
      `ALTER TABLE "notes" ADD CONSTRAINT "FK_notes_mt5_account" FOREIGN KEY ("mt5_account_id") REFERENCES "mt5_accounts"("id") ON DELETE SET NULL ON UPDATE NO ACTION`,
    );
  }

  public async down(queryRunner: QueryRunner): Promise<void> {
    await queryRunner.query(
      `ALTER TABLE "notes" DROP CONSTRAINT "FK_notes_mt5_account"`,
    );
