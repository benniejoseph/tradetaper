...
export class PsychologyAgent extends BaseAgent {
  ...
  readonly capabilities: AgentCapability[] = [
    {
      ...
      description:
        'Analyze trading patterns to identify psychological factors affecting performance',
      keywords: [
        'psychology',
        'behavior',
        'mindset',
  ...
  private async analyzeTraderPsychology(
    ...
    try {
      ...
      const insights =
        await this.psychologyService.analyzePsychologicalPatterns(text);
      ...
      const negativeSentiment = insights.some(
        (i: any) => i.sentiment === 'negative' && i.confidenceScore > 0.7,
      );
    ...
    for (const insight of insights) {
      ...
      if (type.includes('revenge')) {
        recommendations.push(
          'Take a break after losses - implement a mandatory cooling-off period',
        );
      } else if (type.includes('fomo')) {
        recommendations.push(
          'Stick to your trading plan - FOMO entries have lower success rates',
        );
      } else if (type.includes('overtrading')) {
        recommendations.push(
          'Consider reducing trade frequency and focusing on quality setups',
        );
      } else if (type.includes('hesitation')) {
        recommendations.push(
          'Review your entry criteria to build confidence in your setups',
        );
      }
    }
  ...
  private calculateRiskLevel(insights: any[]): 'low' | 'medium' | 'high' {
    ...
    const highRiskCount = insights.filter((i: any) => {
      ...
      return (
        highRiskPatterns.some((p) => type.includes(p)) &&
