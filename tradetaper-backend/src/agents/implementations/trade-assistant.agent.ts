...
export class TradeAssistantAgent extends BaseAgent {
  ...
  readonly capabilities: AgentCapability[] = [
    {
      ...
      keywords: ['predict', 'probability', 'outcome', 'trade', 'setup', 'win', 'loss', 'analysis'],
    },
  ];
  ...
  private async predictTradeOutcome(
    ...
    try {
      ...
      if (!data.instrument || !data.direction || !data.entryPrice || !data.stopLoss || !data.takeProfit) {
        return {
          ...
          error: {
            ...
            message: 'Required: instrument, direction, entryPrice, stopLoss, takeProfit',
          },
        };
      }
      ...
      const prediction = await this.predictionService.generatePrediction(predictionDto);
      ...
      if (prediction.confidence >= 0.8 && prediction.probabilityOfProfit >= 0.7) {
        this.emitAlert({
      ...
      return {
        ...
        data: {
          ...
          recommendation: this.generateRecommendation(prediction, riskRewardRatio),
          sharedState: {
            lastTradePrediction: {
              instrument: data.instrument,
              direction: data.direction,
      ...
      return {
        ...
        error: {
          ...
          message: error instanceof Error ? error.message : 'Trade prediction failed',
        },
      };
    }
  }
  ...
  private generateRecommendation(
    prediction: { probabilityOfProfit: number; predictedOutcome: string; confidence: number },
    ...
    if (prediction.probabilityOfProfit >= 0.7) {
      reasons.push(`High probability of profit (${(prediction.probabilityOfProfit * 100).toFixed(0)}%)`);
    } else if (prediction.probabilityOfProfit < 0.4) {
      reasons.push(`Low probability of profit (${(prediction.probabilityOfProfit * 100).toFixed(0)}%)`);
    }
    ...
    // Determine action
    if (prediction.probabilityOfProfit >= 0.65 && riskRewardRatio >= 1.5 && prediction.confidence >= 0.6) {
      action = 'take';
