...
export class NewsSentimentAgent extends BaseAgent {
  ...
  readonly capabilities: AgentCapability[] = [
    {
      id: 'news-analysis',
      description: 'Analyze recent news and their potential impact on trading instruments',
      keywords: ['news', 'headlines', 'events', 'impact', 'analysis'],
    },
    {
      id: 'sentiment-scoring',
      description: 'Score market sentiment for instruments based on various data sources',
  ...
  private readonly mockNewsData = new Map<string, any[]>([
    ['XAUUSD', [
      { headline: 'Fed signals potential rate pause', impact: 'bullish', weight: 0.7 },
      { headline: 'Dollar weakens on economic data', impact: 'bullish', weight: 0.6 },
      { headline: 'Global uncertainty drives safe-haven demand', impact: 'bullish', weight: 0.8 },
    ]],
    ['EURUSD', [
      { headline: 'ECB maintains hawkish stance', impact: 'bullish', weight: 0.6 },
      { headline: 'US jobs data exceeds expectations', impact: 'bearish', weight: 0.7 },
    ]],
    ['BTCUSD', [
      { headline: 'Institutional adoption continues to grow', impact: 'bullish', weight: 0.8 },
      { headline: 'Regulatory clarity improves in major markets', impact: 'bullish', weight: 0.6 },
      { headline: 'Whale movements detected on-chain', impact: 'neutral', weight: 0.4 },
    ]],
  ]);
  ...
  protected async processMessage(message: AgentMessage): Promise<AgentResponse> {
    ...
    switch (payload.action) {
      ...
      case 'score-sentiment':
        return this.scoreSentiment(payload.symbol || payload.instrument, context);
      
      case 'full-analysis':
        return this.fullSentimentAnalysis(payload.symbol || payload.instrument, context);
  ...
  private async analyzeNews(
    ...
    const overallImpact = netImpact > 0.3 ? 'bullish' : netImpact < -0.3 ? 'bearish' : 'neutral';
    ...
    return {
      ...
      data: {
        ...
        news: news.map(n => ({
          headline: n.headline,
          impact: n.impact,
          significance: n.weight >= 0.7 ? 'high' : n.weight >= 0.5 ? 'medium' : 'low',
        })),
        summary: {
  ...
  private async scoreSentiment(
    ...
    // Weighted average
    const overallScore = (newsScore * 0.4 + socialScore * 0.3 + technicalSentiment * 0.3);
    
    // Determine sentiment level
    let sentimentLevel: 'extreme_fear' | 'fear' | 'neutral' | 'greed' | 'extreme_greed';
  ...
  private async fullSentimentAnalysis(
    ...
    if (sentimentScore > 20 && newsImpact === 'bullish') tradingBias = 'bullish';
    else if (sentimentScore < -20 && newsImpact === 'bearish') tradingBias = 'bearish';
    else if (sentimentScore > 40 || newsImpact === 'bullish') tradingBias = 'bullish';
    else if (sentimentScore < -40 || newsImpact === 'bearish') tradingBias = 'bearish';
    ...
    return {
      ...
      data: {
        ...
        tradingBias,
        recommendation: this.generateSentimentRecommendation(tradingBias, sentimentScore),
      },
    };
  }
  ...
  private interpretSentiment(level: string): string {
    const interpretations: Record<string, string> = {
      'extreme_fear': 'Market is in extreme fear - potential contrarian buy opportunity',
      'fear': 'Bearish sentiment dominates - proceed with caution on longs',
      'neutral': 'Mixed sentiment - wait for clearer signals',
      'greed': 'Bullish sentiment - momentum favors upside but watch for reversals',
      'extreme_greed': 'Market may be overextended - consider taking profits on longs',
    };
