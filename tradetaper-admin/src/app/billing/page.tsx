...
export default function BillingPage() {
  ...
  const { data: subAnalytics, isLoading: subLoading, refetch } = useQuery({
    ...
    enabled: capabilities.canViewBilling,
  });
  ...
  const kpis = [
    ...
    { label: 'Revenue (Est. MRR)', value: totalRevenue > 0 ? `$${totalRevenue.toLocaleString(undefined, { minimumFractionDigits: 2, maximumFractionDigits: 2 })}` : '—', color: '#10B981' },
    { label: 'Conversion Rate', value: totalSubs > 0 ? `${Math.round((paidSubs / totalSubs) * 100)}%` : '—', color: '#22D3EE' },
  ];

  if (adminSession && !capabilities.canViewBilling) {
    return (
        ...
        <div className="flex-1 min-w-0 flex items-center justify-center p-6">
          <div className="admin-card p-6 max-w-md w-full text-center">
            ...
            <p className="text-sm" style={{ color: 'var(--text-muted)' }}>
              Your admin role does not include billing analytics access.
            </p>
          </div>
        </div>
      </div>
        ...
        >
          <div className="max-w-[var(--content-max-width)] mx-auto flex flex-wrap items-center justify-between gap-4">
            <div className="flex items-center gap-3">
              ...
              <div>
                <h1 className="text-2xl font-bold" style={{ color: 'var(--text-primary)' }}>Billing & Revenue</h1>
                <p className="text-sm mt-1" style={{ color: 'var(--text-muted)' }}>Finance operations, revenue health, and subscription performance</p>
              </div>
            </div>
