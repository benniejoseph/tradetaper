          ...
          <div className="bg-blue-900/30 border border-blue-500/30 rounded-lg p-4">
            ...
            {result.user && (
              <div className="text-xs text-gray-400 space-y-1">
                ...
                {result.stats && (
                  <p><strong>Data Created:</strong> {result.stats.trades} trades, {result.stats.accounts} strategies, {result.stats.strategies} strategies, {result.stats.tags} tags</p>
                )}
              </div>
            )}
          </div>
    ...
    <div className="bg-gray-900/80 backdrop-blur-xl border border-gray-700/50 rounded-2xl p-6">
      <div className="flex items-center justify-between mb-6">
        ...
        <div className="flex items-center space-x-2">
          <button
             onClick={onAdd}
            className="p-2 bg-blue-600 hover:bg-blue-700 text-white rounded-lg transition-all flex items-center space-x-2"
          >
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 backdrop-blur-sm">
      ...
      >
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          {columns.map((col) => {
            // Skip system columns
            if (['created_at', 'updated_at', 'createdAt', 'updatedAt'].includes(col.column_name)) {
              return null;
            }

            // Skip ID and createdAt/updatedAt for creation usually, but let's be flexible
            // If it's serial/auto-increment, we probably shouldn't edit it.
            // For now, we show all but maybe disable ID on edit.
            const isReadOnly = mode === 'edit' && col.column_name === 'id';
            
            return (
              <div key={col.column_name}>
                <label className="block text-sm font-medium text-gray-400 mb-1">
                  {col.column_name} <span className="text-gray-600 text-xs">({col.data_type})</span>
                </label>
                <input
                  type={col.data_type.includes('int') || col.data_type.includes('float') || col.data_type.includes('numeric') ? 'number' : 'text'}
...
function DatabasePageContent() {
  ...
  const handleDeleteRow = async (row: any) => {
    ...
    try {
      await adminApi.deleteDatabaseRow(selectedTable, row.id);
      loadTableData();
    } catch (error: any) {
  ...
  const handleSaveRow = async (formData: any) => {
    ...
    if (modalMode === 'create') {
      await adminApi.createDatabaseRow(selectedTable, formData);
    } else {
      if (!editingRow.id) throw new Error('Cannot edit row without ID');
      await adminApi.updateDatabaseRow(selectedTable, editingRow.id, formData);
    }
    loadTableData();
  };
