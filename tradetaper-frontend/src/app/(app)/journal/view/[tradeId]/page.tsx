...
import TradeShareModal from '@/components/journal/TradeShareModal';
...
const cardClass =
  'min-w-0 rounded-2xl border border-zinc-200/70 bg-white/80 px-4 py-3 dark:border-white/10 dark:bg-white/[0.02]';
...
const InlineMetric: React.FC<{
  ...
  return (
    <div className="flex items-start justify-between gap-4 py-1.5">
      <span className="shrink-0 text-xs uppercase tracking-[0.12em] text-zinc-500 dark:text-zinc-400">{label}</span>
      <span className={`max-w-[68%] break-words text-right text-sm font-semibold leading-tight ${valueColor}`}>
        {value}
      </span>
    </div>
  );
...
export default function ViewTradePage() {
  ...
  const swapValue = trade?.swap;

  const tags = useMemo(() => {
    const rawTags = (trade?.tags || []) as Array<string | { name?: string }>;
    return rawTags
      .map((tag) => (typeof tag === 'string' ? tag : tag?.name))
      .filter((tag): tag is string => Boolean(tag && tag.trim()));
  }, [trade?.tags]);

  const isTradeMismatch = Boolean(trade && trade.id !== tradeId);

  if (isLoading || (isTradeMismatch && !error)) {
    return (
  ...
  if (error || !trade || isTradeMismatch) {
    return (
      <div className="min-h-[60vh] flex items-center justify-center">
      ...
      <div className={`${panelClass} relative overflow-hidden p-4 md:p-6`}>
        ...
        <div className="relative z-10 space-y-5">
          <div className="flex flex-col gap-3 lg:flex-row lg:items-center lg:justify-between">
            <div className="flex flex-wrap items-center gap-2">
              ...
              <button
                type="button"
                disabled
                className="inline-flex cursor-not-allowed items-center gap-2 rounded-lg border border-zinc-300/70 bg-zinc-100/70 px-2.5 py-1.5 text-xs font-semibold text-zinc-500 opacity-70 dark:border-white/15 dark:bg-white/[0.03] dark:text-zinc-400"
                title="Replay this trade is coming soon"
              >
                Replay This Trade
                <span className="rounded-md border border-emerald-500/35 bg-emerald-500/10 px-1.5 py-0.5 text-[10px] font-bold uppercase tracking-[0.08em] text-emerald-600 dark:text-emerald-300">
                  Coming soon
                </span>
              </button>
          ...
          <div className="flex flex-col gap-5 xl:flex-row xl:items-end xl:gap-6">
            ...
            <div className="grid w-full grid-cols-2 gap-3 sm:grid-cols-4 xl:w-[min(760px,52vw)] xl:flex-none">
              <div className={cardClass}>
                <p className="text-[10px] uppercase tracking-[0.14em] text-zinc-500 dark:text-zinc-400">Total P&L</p>
                <p className={`mt-2 break-words text-[clamp(1.25rem,1.7vw,2.2rem)] font-black leading-none ${isWin ? 'text-emerald-500 dark:text-emerald-400' : 'text-red-500 dark:text-red-400'}`}>
              ...
              <div className={cardClass}>
                ...
                <p className={`mt-3 break-words text-[clamp(1.1rem,1.35vw,1.95rem)] font-black leading-none ${pnlPercentage !== null && pnlPercentage >= 0 ? 'text-emerald-500 dark:text-emerald-400' : 'text-red-500 dark:text-red-400'}`}>
              ...
              <div className={cardClass}>
                ...
                <p className="mt-3 break-words text-[clamp(1.1rem,1.35vw,1.95rem)] font-black leading-none text-emerald-500 dark:text-emerald-400">
              ...
              <div className={cardClass}>
                ...
                <p className="mt-3 break-words text-[clamp(1.1rem,1.35vw,1.95rem)] font-black leading-none text-zinc-900 dark:text-white">
          ...
          <div className="grid grid-cols-1 gap-5 xl:grid-cols-[minmax(0,1fr)_360px]">
            <div className="space-y-5">
              ...
              <div className={sectionCardClass}>
                ...
                <div className="relative overflow-hidden rounded-2xl border border-zinc-200 bg-zinc-100/60 dark:border-white/10 dark:bg-white/[0.02]">
                  <div className="pointer-events-none absolute inset-0 bg-[linear-gradient(135deg,rgba(148,163,184,0.15),transparent_45%,rgba(148,163,184,0.2))] dark:bg-[linear-gradient(135deg,rgba(255,255,255,0.05),transparent_45%,rgba(255,255,255,0.08))]" />
                  <div className="h-[460px] md:h-[520px] flex items-center justify-center p-6 text-center grayscale">
                    <div className="space-y-3">
                      ...
                      <p className="text-sm font-semibold uppercase tracking-[0.12em] text-zinc-600 dark:text-zinc-300">
                        Price Action / Trade Flow
                      </p>
            ...
            <div className="space-y-5">
              <div className={sectionCardClass}>
                ...
                <InlineMetric label="Swap" value={formatCurrency(swapValue)} emphasis={(swapValue || 0) < 0 ? 'red' : 'neutral'} />
