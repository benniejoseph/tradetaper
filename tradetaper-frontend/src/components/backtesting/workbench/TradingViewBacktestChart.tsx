...
import { CandleData } from './mockData';
...
>(function TradingViewBacktestChart(
  ...
  const oneMinuteCandlesRef = useRef<CandleData[]>(oneMinuteCandles);
  const previousOneMinuteCountRef = useRef<number>(oneMinuteCandles.length);
  ...
  oneMinuteCandlesRef.current = oneMinuteCandles;
  ...
  const datafeed = useMemo(
    () =>
      createTradingViewBacktestingDatafeed({
        ...
        getOneMinuteBars: () => oneMinuteCandlesRef.current,
      }),
    [apiBase],
  );
  ...
  const saveLayoutToBackend = useCallback(
    async (layout: Record<string, unknown> | null) => {
      ...
      await fetch(`${apiBase}/replay/sessions/${sessionId}/chart-layout`, {
        method: 'PUT',
        credentials: 'include',
        headers: {
  ...
  useEffect(() => {
    const widget = widgetRef.current;
    if (!widget || !chartReady) return;
    const nextCount = oneMinuteCandles.length;
    const prevCount = previousOneMinuteCountRef.current;
    previousOneMinuteCountRef.current = nextCount;

    if (nextCount === prevCount) return;
    widget.activeChart?.().resetData?.();
  }, [chartReady, oneMinuteCandles.length]);
  ...
  useEffect(() => {
    ...
    const loadSavedLayout = async () => {
      ...
      try {
        const res = await fetch(`${apiBase}/replay/sessions/${sessionId}/chart-layout`, {
          method: 'GET',
          credentials: 'include',
          headers: {
            'Content-Type': 'application/json',
