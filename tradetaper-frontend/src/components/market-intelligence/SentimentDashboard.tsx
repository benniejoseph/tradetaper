            ...
            <div className="bg-white dark:bg-black/70 rounded-xl p-5 border border-gray-100 dark:border-emerald-900/40">
              ...
              <div className="space-y-3 text-xs text-gray-600 dark:text-gray-300">
                {report.economicEvents.map((event) => (
                  <div key={event.id} className="flex flex-col gap-2 sm:flex-row sm:items-start sm:justify-between">
                    <div>
                    ...
                    <span className={`self-start sm:self-auto px-2 py-1 rounded-full text-[10px] uppercase ${event.importance === 'high' ? 'bg-red-100 text-red-600' : event.importance === 'medium' ? 'bg-amber-100 text-amber-600' : 'bg-emerald-100 text-emerald-600'}`}>
                      {event.importance}
                    </span>
                  </div>
                ))}
