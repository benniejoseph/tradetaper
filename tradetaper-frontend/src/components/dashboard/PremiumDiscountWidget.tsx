...
export default function PremiumDiscountWidget({ symbol = 'XAUUSD' }: Props) {
  ...
  const generateDemoData = (sym: string): PremiumDiscountData => {
    ...
    return {
      ...
      currentPrice: price,
      fibonacci: [
        { level: 0, price: low, label: '0% (Swing Low)', zone: 'discount' },
        { level: 23.6, price: low + range * 0.236, label: '23.6%', zone: 'discount' },
        { level: 38.2, price: low + range * 0.382, label: '38.2%', zone: 'discount' },
        { level: 50, price: low + range * 0.5, label: '50% (EQ)', zone: 'equilibrium' },
        { level: 61.8, price: low + range * 0.618, label: '61.8% (OTE)', zone: 'premium' },
        { level: 78.6, price: low + range * 0.786, label: '78.6% (OTE)', zone: 'premium' },
        { level: 100, price: high, label: '100% (Swing High)', zone: 'premium' },
      ],
      optimalTradeEntry: {
        min: low + range * 0.618,
        max: low + range * 0.786,
