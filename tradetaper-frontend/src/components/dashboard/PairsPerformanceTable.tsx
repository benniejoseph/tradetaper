...
export default function PairsPerformanceTable({ data }: PairsPerformanceTableProps) {
  return (
    <div className="bg-gradient-to-br from-white to-emerald-50 dark:from-black dark:to-emerald-950/20 backdrop-blur-xl rounded-2xl border border-emerald-200/50 dark:border-emerald-700/30 shadow-lg overflow-hidden">
      ...
      {data.length > 0 ? (
        <>
          <div className="md:hidden divide-y divide-emerald-200/30 dark:divide-emerald-700/30">
            {data.map((pair) => (
              <div key={pair.symbol} className="p-4 space-y-3">
                <div className="flex items-start justify-between gap-3">
                  <p className="text-sm font-semibold text-gray-900 dark:text-white">{pair.symbol}</p>
                  <p
                ...
                <div className="grid grid-cols-2 gap-x-4 gap-y-2 text-xs">
                  ...
                  <div className="text-gray-500 dark:text-gray-400">MAE</div>
                  <div className="text-right font-mono text-red-600 dark:text-red-400">
                    ${Math.abs(pair.mae).toFixed(2)}
                  </div>
                  <div className="text-gray-500 dark:text-gray-400">Win %</div>
                  <div
                  ...
                  <div className="text-right text-gray-700 dark:text-gray-300">{pair.tradesCount}</div>
                </div>
              </div>
            ))}
          </div>
